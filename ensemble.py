    ...
    while current_predict_start <= df.index.max():        
        ...
        for model_name, model in models.items():
            try:
                ...
                y_pred_shifted = pd.Series(y_pred, index=predict.index).shift().fillna(1)
                ...
                strategy_model_dfs[strategy][model_name].loc[predict.index, f'Preds_{model_name}'] = y_pred_shifted
            except:
                continue      
...
for strategy in strategies:
    try:
        ...
        pred_zero_cb = df.loc[
            (df.index >= start) & (df.index <= "2024-12-31") & (df['Preds_CatBoost'] == 0)
        ].index
        pred_zero_lgbm = df.loc[
            (strategy_model_dfs[strategy]['LightGBM'].index >= start) & (strategy_model_dfs[strategy]['LightGBM'].index <= "2024-12-31") & (strategy_model_dfs[strategy]['LightGBM']['Preds_LightGBM'] == 0)
        ].index
        ...
        sharpe_negative_dates = test_period.loc[test_period["Sharpe_Ratio_Target"]==0].index
        ...
        ax1.plot(test_period.index, cumulative_returns, label="Cumulative Returns", color="blue")
        ax1.scatter(pred_zero_cb, cumulative_returns.loc[pred_zero_cb], 
                    color="yellow", label="Predicted 0", zorder=5)
        ax1.scatter(pred_zero_lgbm, cumulative_returns.loc[pred_zero_lgbm], 
                    color="green", label="Predicted 0", zorder=5, marker='v')
        
        ax1.plot(test_period.index, cumulative_sim_cb, label="Simulated (CatBoost)", linestyle=":", color="orange")
        ax1.plot(test_period.index, cumulative_sim_lgbm, label="Simulated (LightGBM)", linestyle="--", color="purple")

        ax1.scatter(sharpe_negative_dates, cumulative_returns.loc[sharpe_negative_dates],
                    color="red", label="Sharpe Ratio < 0", zorder=5, marker="x")
        ax1.set_title(f"{strategy} - Cumulative Returns with Predictions (2024)")
        ax1.set_ylabel("Cumulative Returns")
        ax1.grid(True)
        ax1.legend()
        ax1.text(0.95, 0.95, f"Autocorrelation: {autocorrelation:.2f}\nSharpe Ratio: {sharpe_ratio:.2f}",
                 transform=ax1.transAxes, fontsize=12, verticalalignment="top", horizontalalignment="right",
                 bbox=dict(facecolor="white", alpha=0.6))
    
        ax2.plot(test_period.index, test_period["Sharpe_Ratio_30"], label="Sharpe Ratio (30)", color="green")

...
def calculate_ewma_volatility(returns, span=30, lambda_=0.97, trading_days=252):
    ewma_variance = returns.ewm(span=span, adjust=False).mean()
    ewma_volatility = np.sqrt(ewma_variance) * np.sqrt(trading_days)
...
def calculate_parkinson_volatility(df, window=30, trading_days=252):
    ...
    parkinson_volatility = np.sqrt(rolling_parkinson_var) * np.sqrt(trading_days)
...
def calculate_garman_klass_volatility(df, window=30, trading_days=252):
    ...
    gk_volatility = np.sqrt(rolling_gk_var) * np.sqrt(trading_days)
...
def calculate_max_drawdown_duration(cum_returns):
    '''
    maximum duration of drawdowns over a 30-day rolling window for strategy returns
    '''
...
def calculate_volatility_of_volatility(volatility, window=30):
    '''
    standard deviation of the average returns volatility over a 30-day rolling window
    '''
    vol_of_vol = volatility.rolling(window).std()
    return vol_of_vol


def process_volatility_data(ticker, data_path, windows, lambda_, trading_days=252):
    ...
    for window in windows:
        ...
        volatility_df[f"Realized_Volatility_{window}"] = calculate_realized_volatility(
            df, window=window, trading_days=trading_days
        )
        
        volatility_df[f"EWMA_Volatility_{window}"] = calculate_ewma_volatility(
            df_daily["Log_Returns"], span=window, lambda_=lambda_, trading_days=trading_days
        )
    ...
    all_features = volatility_df.copy()
    
    for window in windows:
        macd_df = calculate_macd(df_daily)
        all_features[f"MACD_{window}"] = macd_df['macd']
        all_features[f"MACD_Signal_{window}"] = macd_df['macd_signal']
        all_features[f"MACD_Diff_{window}"] = macd_df['macd_diff']
        
        all_features[f"RSI_{window}"] = calculate_rsi(df_daily, window=window)
        all_features[f"ADX_{window}"] = calculate_adx(df_daily, window=window)
        all_features[f"Momentum_{window}"] = calculate_momentum(df_daily, window=window)
        all_features[f"Mean_Returns_{window}"] = calculate_mean_returns(df_daily["Log_Returns"], window=window)
        all_features[f"Sharpe_Ratio_{window}"] = calculate_sharpe_ratio(
            df_daily["Log_Returns"], window=window, risk_free_rate=0.0, trading_days=trading_days
        )
        
        skew_kurt = calculate_skewness_kurtosis(df_daily["Log_Returns"], window=window)
        all_features[f"Skewness_{window}"] = skew_kurt['skewness']
        all_features[f"Kurtosis_{window}"] = skew_kurt['kurtosis']
        
        all_features[f"Hurst_{window}"] = df_daily['close'].rolling(window).apply(compute_hurst_exponent, raw=False)
        all_features[f"OU_Theta_{window}"] = df_daily['close'].rolling(window).apply(compute_ou_theta, raw=False)
        all_features[f"Hurst_Returns_{window}"] = df_daily['Log_Returns'].rolling(window).apply(compute_hurst_returns, raw=False)
        
        for lag in range(1, 11):
            all_features[f"ACF_Lag_{lag}_{window}"] = df_daily["Log_Returns"].rolling(window).apply(lambda x: x.autocorr(lag=lag), raw=False)
    
    all_features["Cumulative_Returns"] = calculate_cumulative_returns(df_daily["Log_Returns"])
    all_features["Log_Returns"] = df_daily["Log_Returns"]
    
    all_features["Max_Drawdown_Duration_30"] = calculate_max_drawdown_duration(all_features["Cumulative_Returns"]).rolling(window=30).max()
    all_features["Volatility_of_Volatility_30"] = calculate_volatility_of_volatility(all_features["Historical_Volatility_30"], window=30)
    ...
    # можно добавить отдельно отсеивание некорректных значений
...
def process_rets_data(rets_path, output_path, windows, trading_days=252):
    ...
    for strategy in strategies:
        ...
        for window in windows:
            features_df[f"Mean_Returns_{strategy}_{window}"] = calculate_mean_returns(returns, window=window)
            features_df[f"Sharpe_Ratio_{strategy}_{window}"] = calculate_sharpe_ratio(
                returns, window=window, risk_free_rate=0.0, trading_days=trading_days
            )
            
            skew_kurt = calculate_skewness_kurtosis(returns, window=window)
            features_df[f"Skewness_{strategy}_{window}"] = skew_kurt['skewness']
            features_df[f"Kurtosis_{strategy}_{window}"] = skew_kurt['kurtosis']
            
            features_df[f"Hurst_{strategy}_{window}"] = returns.rolling(window).apply(compute_hurst_returns, raw=False)
            
            for lag in range(1, 11):
                features_df[f"ACF_Lag_{lag}_{strategy}_{window}"] = returns.rolling(window).apply(lambda x: x.autocorr(lag=lag), raw=False)
    ...
    features_df["Max_Drawdown_Duration_30"] = calculate_max_drawdown_duration(features_df["Cumulative_Returns"]).rolling(window=30).max()
...
def main():
    ...
    for ticker in tickers:
        ...
        if os.path.isfile(ticker_file):
            process_volatility_data(ticker, data_path, windows, lambda_, trading_days)
        else:
